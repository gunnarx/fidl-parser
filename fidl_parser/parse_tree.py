...
def process_lark_tree_entry(state, lark_tree, lark_entry, parse_map):
    ...
    log(f"{ind()}Called for {lark_tree.data}[{lark_entry}]")

    entries = get_lark_tree_entries_by_name(state, lark_tree, lark_entry)
    if entries == []:
        log(f"{ind()}No match for {lark_entry}. Return False.")
        return None
...
def create_entry(state, lark_tree, lark_type, target_name, target_type=str):
    ...
    if not res:
        log(f"{ind()}Not found: {lark_type}")
        return None

    log(f"{ind()}Found {lark_type} = {res} -> {target_name}")
...
def create_target_dictionary(state, lark_tree, lark_entry, target_entry, parse_map):
    log(f"{ind()}Called for {lark_tree.data}[{lark_entry}] -> {target_entry}")
    res = process_lark_tree_entry(state, lark_tree, lark_entry, parse_map)
    if not res:
        log(f"{ind()}Got None back. No action.")
        return None

    log(f"{ind()}Create new {target_entry} dictionary with: {type(res)} - {len(res)} elements.")
...
def aggregate_list(state, lark_tree, element_name, parse_map):
    ...
    log(f"{ind()}Will aggregate {len(result)} elements into {element_name}")
...
def create_entry_from_type_token(state, lark_tree, target_entry):
    ...
    log(f"{ind()}Called for {lark_tree.data} -> {target_entry}")
    if len(lark_tree.children) == 0:
        log(f"{ind()}{lark_tree.data} has no tokens. No action")
        return None
...
def process_one_of(state, lark_tree, options):
    log(f"{ind()}Called for {lark_tree.data}")
...
def add_datatype(state, lark_elem, parse_map):
    ...
    log(f"{ind()}ADDING DATATYPE---------------- {res['name']}")
...
def evaluate_expression(state, lark_tree, expr_name, target_name):
    ...
    def eval_token(expr_token):
        log(f"{ind()}Evaluating {expr_token}")


    @indent
    def eval_tree(expr_tree):
        log(f"{ind()}Evaluting {expr_tree.data}")

        for expr in expr_tree.children:
            log(f"{ind()}Checking {expr}")
            if isinstance(expr, lark.Token):
                if expr.type in token_map:
                    log(f"{ind()}Evaluating token {expr.type} = {expr.value}")
                    return token_map[expr.type](expr.value)
                else:
                    log(f"{ind()}No token lambda available to evaluate {expr.type}")
                    continue
            else:
                if expr.data in tree_map:
                    log(f"{ind()}Evaluating tree {expr.data}")
                    return tree_map[expr.data](expr)
                else:
                    log(f"{ind()}No tree lambda available to evaluate {expr.data}")
                    continue
    ...
    def eval_expression(expr_tree, expr_name):
        log(f"{ind()}Searching for {expr_name} in {expr_tree.data}")
    ...
    log(f"{ind()}Evaluating {lark_tree.data}/{expr_name}")
...
def resolve_types(current_namespace, name, tree):
    ...
    log(f"{ind()}Resolving {name}")

    if 'datatype' in tree:
        log(f"{ind()}Found datatype")
